...
void compra(int opc, float &cred, int stockArray[], float &mochila, int &cupoes, int &Nivel, int &xp,int XpPorProduto[]) {
    ...
    if (mochila <= 0) {
        cout << "\nNao tens espaco na mochila para carregar mais nada.\n";
...
float Descontototal = produtos * precosDesconto;
    while (true) {
        ...
        cin >> produtos;
        float TotalTamanho = produtos * ProdutosTamanho[opc];
        ...
        cout << "\nPor $" << total <<" e " << TotalTamanho <<  ". Confirmas a compra? (s/n): ";
        cin >> decisao;

        if (decisao == 's' || decisao == 'S') {
            cred -= total;
            stockArray[opc] -= produtos;
            mochila -= TotalTamanho;
            xp += XpPorProduto[opc];
